...
const getAllTaskLists = (client: IndexDBClient) => async (): Promise<TaskListsResponse> => {
  ...
  const listsWithTasksMeta = await Promise.all(
    lists.map(async (list) => {
      const allTasks = await db.transaction('tasks').store.index('by-list').getAll(list.id)
...
const getTaskList =
  ...
  async (id: string, filters?: TasksFilterOptions): Promise<TaskListResponse> => {
    const db = client || (await getDB())
    ...
    const allTasks = await db.transaction('tasks').store.index('by-list').getAll(id)
